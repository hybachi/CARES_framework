"""
nav2_bridge.py
Translates CHROMA high-level task waypoints into ROS 2 Nav2 goals.
...
class Nav2Bridge(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().info(f"[{self.robot_id}] Nav2 Bridge Active.")
    ...
    def setup_pubs_subs(self):
        ...
        self.create_subscription(Task, 'cancel_waypoint', self.cancel_cb, 10)
    ...
    def task_cb(self, msg):
        if self.active_task:
            self.get_logger().warn("Execution ignored; bridge is busy.")
            return
        ...
        self.get_logger().info(
            f"Navigating to ({msg.location.x:.2f}, {msg.location.y:.2f}) "
            f"facing {math.degrees(yaw):.0f} deg."
        )
    ...
    def cancel_cb(self, msg):
        ...
        self.get_logger().warn("Halt command received. Canceling Nav2 goal.")
